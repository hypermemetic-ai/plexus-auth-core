...
string_newtype! {
    /// Fully-qualified method path, e.g. `auth.login`. Local stub for the
    /// `MethodPath` newtype pinned by AUTHZ-S01-output §1. When the upstream
    /// `MethodPath` lands in a shared crate this becomes a `pub use` re-export.
    MethodPath
}

string_newtype! {
    /// HTTP header name, e.g. `authorization`. Local stub for the canonical
    /// `HeaderName` newtype pinned by CLIENTS-S01-output §1.
    HeaderName
}

string_newtype! {
    /// HTTP cookie name, e.g. `plexus_session`. Local stub for the canonical
    /// `CookieName` newtype pinned by CLIENTS-S01-output §1.
    CookieName
}
...
mod tests {
    ...
    fn sample_metadata() -> CredentialMetadata {
        CredentialMetadata::new(
            ...
            AttachmentSite::Header {
                name: HeaderName::new("authorization"),
            },
            Some(CredentialScheme::new("Bearer ")),
            vec![Scope::new("cone.send_message")],
            None,
            Some(MethodPath::new("auth.refresh")),
            Some(MethodPath::new("auth.logout")),
            sample_issuer(),
        )
    }
    ...
    fn attachment_site_variants_round_trip() {
        ...
        let variants = vec![
            ...
            AttachmentSite::Cookie {
                name: CookieName::new("plexus_session"),
            },
            AttachmentSite::FirstFrame {
                setup_method: MethodPath::new("auth.connect"),
    ...
    fn newtypes_serialize_transparently() {
        ...
        let h = serde_json::to_string(&HeaderName::new("authorization")).unwrap();
